...
class Settings(BaseSettings):
    ...
    # --- Массовые фоновые рассылки (winback/welcome_nudge) — см. диалог
    # 2026-08-27, реальный инцидент при cutover: сразу на старте, ДО первой
    # проверки, что бот вообще корректно поднялся, эти циклы разослали
    # уведомления части из 7599 юзеров, мигрированных со старого бота (winback_sent/
    # welcome_nudge_sent пусты после переезда — весь массив подряд считается
    # "ещё не уведомлённым"). По умолчанию — включено (для свежих, не
    # мигрированных инсталляций это штатное поведение); при развёртывании
    # поверх мигрированной базы — держать выключенным, пока критерии не
